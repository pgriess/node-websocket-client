...
var WebSocket = function(url, proto, opts) {
    ...
    (function() {
        ...
        //httpHeaders['Sec-WebSocket-Version'] = "7";
        ...
        if(opts.credentials) {
            httpHeaders.Authorization = 'Basic ' + 
                new Buffer( opts.credentials.username + ':' + 
                            opts.credentials.password).toString('base64');
        }
        var default_port = 80;
        var urlScheme = getUrlScheme(url);
        ...
        case 'wss':
            ...
            if (+/^v0.([0-9]+).[0-9]+$/.exec(process.version)[1] <= 4) {
                ...
                if(options.secure)
                httpClient = https.request(options);
                else
