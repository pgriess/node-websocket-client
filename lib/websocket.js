...
var WebSocket = function(url, proto) {
    ...
    var loFrameParser = function(buf, off) {
    ...
    var hiFrameParser = function(buf, off) {
    ...
    // Figure out what frame type we are
    var unknownFrameParser = function(buf, off) {
        if (buf[off] & 0x80) {
    ...
    var dataListener = function(buf) {
        ...
        do {
            switch (frameType) {
            case FRAME_NO:
                consumed = unknownFrameParser(buf, off);
                break;

            case FRAME_LO:
                consumed = loFrameParser(buf, off);
                break;
