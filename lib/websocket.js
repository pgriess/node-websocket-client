...
var WebSocket = function(url, proto) {
    ...
    // Finish handling a frame
    //
    // This code handles emitting a message and fixing up the framing metadata
    // above with any remaining data in the buffer.
    var finishFrame = function(msgBuf, buf, i) {
        if (i < (buf.length - 1)) {
            bufs.push(buf.slice(i + 1, buf.length));
            bufsBytes = bufs[0].length;
        }

        self.emit('message', msgBuf);
    };

    // Low-byte frame parser
    var loFrameParser = function(buf) {
        assert.ok(bufs.length > 0 || bufsBytes == 0);

        // Loop over data while we have any outstanding bytes that we have not
        // yet looked at
        while (buf.length > 0) {

            // Find the first instance of 0xff, our terminating byte
            for (var i = 0; i < buf.length && buf[i] != 0xff; i++)
                ;

            // We didn't find a terminating byte; queue up this buffer for later
            // and call it a day
            if (i >= buf.length) {
                bufs.push(buf);
                bufsBytes += buf.length;
                break;
            }

            // We found a terminating byte; collect all bytes into a single
            // buffer and emit it
            var mb = null;
            if (bufs.length == 0) {
                mb = buf.slice(0, i);
            ...
            // No more data in this buffer; we're done
            if (i == (buf.length - 1)) {
                break;
            }

            // We have more data; set up our loop
            buf = buf.slice(i + 1, buf.length);
        }
    };

    // High-byte framing parser
    var hiFrameParser = function(buf) {
    ...
    var dataListener = function(buf) {
        ...
        case FRAME_NO:
            if ((buf[0] & 0x80) == 0x00) {
                frameType = FRAME_LO;
                loFrameParser(buf.slice(1, buf.length));
            } else {
                sys.debug('High-byte framing not yet supported');
            }
        ...
        case FRAME_LO:
            return loFrameParser(buf);
            break;

        case FRAME_HI:
            return hiFrameParser(buf);
            break;

        default:
            sys.debug('Invalid frame type ' + frameType + '; resetting');
            frameType = FRAME_NO;
            break;
        }
    };
